...
const api: ZnxStudioApi = {
  ...
  mobile: {
    ...
    profileStart: (config) => ipcRenderer.invoke(IpcChannels.MobileProfileStart, config),
    profileStop: () => ipcRenderer.invoke(IpcChannels.MobileProfileStop),
    onProfileEvent: (callback) => subscribe<MobileProfileEvent>(IpcChannels.MobileProfileEvent, callback),
    buildApk: (config) => ipcRenderer.invoke(IpcChannels.MobileBuildApk, config),
    buildAab: (config) => ipcRenderer.invoke(IpcChannels.MobileBuildAab, config),
    buildStop: () => ipcRenderer.invoke(IpcChannels.MobileBuildStop),
    onBuildProgress: (callback) => subscribe<MobileBuildProgress>(IpcChannels.MobileBuildProgress, callback),
    releaseCheck: (workspaceRoot) => ipcRenderer.invoke(IpcChannels.MobileReleaseCheck, workspaceRoot),
    clean: (workspaceRoot) => ipcRenderer.invoke(IpcChannels.MobileClean, workspaceRoot),
    sessionState: () => ipcRenderer.invoke(IpcChannels.MobileSessionState),
    onSessionState: (callback) => subscribe<MobileSessionState>(IpcChannels.MobileSessionState, callback),
    projectConfig: (workspaceRoot) => ipcRenderer.invoke(IpcChannels.MobileProjectConfig, workspaceRoot),
    updateProjectConfig: (workspaceRoot, config) => ipcRenderer.invoke(IpcChannels.MobileProjectConfigUpdate, workspaceRoot, config),
  },
  androidToolchain: {
    status: () => ipcRenderer.invoke(IpcChannels.AndroidToolchainStatus),
    setup: () => ipcRenderer.invoke(IpcChannels.AndroidToolchainSetup),
    onSetupProgress: (callback) => subscribe<ToolchainSetupProgress>(IpcChannels.AndroidToolchainSetupProgress, callback),
    sdkList: () => ipcRenderer.invoke(IpcChannels.AndroidToolchainSdkList),
